CREATE TYPE "public"."status" AS ENUM('pending', 'scraped', 'splitted', 'embedded', 'completed');--> statement-breakpoint
CREATE TABLE "bookmark_content" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"content" text NOT NULL,
	"content_hash" varchar(64) NOT NULL,
	CONSTRAINT "bookmark_content_content_hash_unique" UNIQUE("content_hash")
);
...
CREATE TABLE "user_feedback" (
	...
	"feedback" text,
	CONSTRAINT "rating_check" CHECK ("user_feedback"."rating" >= 1 AND "user_feedback"."rating" <=5)
);
--> statement-breakpoint
...
ALTER TABLE "user_feedback" ADD CONSTRAINT "user_feedback_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
